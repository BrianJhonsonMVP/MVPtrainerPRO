    ...
    <div className="animate-fadeIn max-w-4xl mx-auto pb-20 relative">
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3 mb-6">
        ...
        <div className="flex gap-2">
          ...
          <span className="font-bold text-white px-4 py-2 bg-zinc-900 rounded-lg capitalize text-sm sm:text-base">
      ...
      <div className={`grid grid-cols-7 gap-1 sm:gap-2 mb-2 text-center text-[10px] sm:text-xs font-bold text-zinc-500 uppercase tracking-wider ${!isPro ? 'opacity-40 select-none' : ''}`}>
        {['Lun', 'Mar', 'Mié', 'Jue', 'Vie', 'Sáb', 'Dom'].map(d => <div key={d}>{d}</div>)}
      </div>

      <div className={`grid grid-cols-7 gap-1 sm:gap-2 ${!isPro ? 'opacity-40 select-none' : ''}`}>
        {blanksArray.map(b => <div key={`blank-${b}`} className="h-20 sm:h-24 bg-transparent"></div>)}
        ...
        {daysArray.map(day => {
          ...
          return (
            <div key={day} className={`h-20 sm:h-24 bg-zinc-900 rounded-lg sm:rounded-xl border p-1.5 sm:p-2 flex flex-col ${isToday ? 'border-mvp-gold bg-mvp-gold/5' : 'border-zinc-800'}`}>
              <span className={`text-xs sm:text-sm font-bold mb-1 ${isToday ? 'text-mvp-gold' : 'text-zinc-400'}`}>{day}</span>
        ...
        <div className="absolute inset-0 flex items-center justify-center z-10 px-4">
          <div className="bg-black/80 backdrop-blur-md p-8 rounded-3xl border border-mvp-gold/30 shadow-2xl text-center max-w-sm">
            ...
            <p className="text-zinc-400 text-sm mb-6">
              Visualiza vencimientos de pagos, estados de deuda y tu ingreso mensual estimado.
            </p>
